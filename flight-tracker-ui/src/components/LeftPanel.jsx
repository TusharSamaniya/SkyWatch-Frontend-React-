...
const LeftPanel = ({ activeTool, onClose, airports, flights = [], onDrawArc, onAirlineSelect, activeAirline, userLocation, onDetectLocation }) => {
  ...
  return (
    <div style={{
      ...
      {activeTool === 'route' && (
        <div style={{ marginTop: '20px' }}>
          <div style={{ display: 'flex', gap: '10px', marginBottom: '15px' }}>
            <input value={depIata} onChange={(e) => setDepIata(e.target.value.toUpperCase())} placeholder="Origin" style={{ flex: 1, background: '#1f2937', border: '1px solid #374151', color: 'white', padding: '10px', borderRadius: '5px', textTransform: 'uppercase' }} />
            <span style={{ paddingTop: '10px', color: '#9ca3af' }}>➔</span>
            <input value={arrIata} onChange={(e) => setArrIata(e.target.value.toUpperCase())} placeholder="Dest" style={{ flex: 1, background: '#1f2937', border: '1px solid #374151', color: 'white', padding: '10px', borderRadius: '5px', textTransform: 'uppercase' }} />
          </div>
      ...
      {activeTool === 'airline' && (
        <div style={{ marginTop: '20px' }}>
          <p style={{ color: '#9ca3af', fontSize: '14px', marginBottom: '15px' }}>Isolate specific airlines to view their active global fleet.</p>
          <input type="text" placeholder="Search Airline (e.g., IndiGo)" value={airlineSearch} onChange={(e) => setAirlineSearch(e.target.value)} style={{ width: '100%', background: '#1f2937', border: '1px solid #374151', color: 'white', padding: '12px', borderRadius: '5px', marginBottom: '20px', boxSizing: 'border-box' }} />
          {isLoading ? (
            <div style={{ color: '#ffba00', textAlign: 'center', marginTop: '20px' }}>Downloading Global Fleet DB...</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <button onClick={() => onAirlineSelect(null)} style={{ background: activeAirline === null ? '#10b981' : '#1f2937', color: activeAirline === null ? 'black' : 'white', border: 'none', padding: '12px', borderRadius: '5px', cursor: 'pointer', fontWeight: 'bold', textAlign: 'left' }}>🌍 Show All Airlines</button>
      ...
      {activeTool === 'delays' && (
        <div style={{ marginTop: '20px' }}>
          <p style={{ color: '#9ca3af', fontSize: '14px', marginBottom: '15px' }}>Real-time tracker of the most delayed flights currently in the air.</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {delayedFlightsList.length === 0 ? (
              <div style={{ color: '#10b981', textAlign: 'center', padding: '20px', background: '#1f2937', borderRadius: '8px' }}>✅ No significant delays detected in this region right now!</div>
            ) : (
              delayedFlightsList.map((flight, idx) => (
                <div key={idx} style={{ background: '#1f2937', padding: '15px', borderRadius: '8px', borderLeft: '4px solid #ef4444', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <div style={{ fontWeight: 'bold', color: '#f87171', fontSize: '16px' }}>{flight.airline_iata || ''} {flight.flight_number || flight.callsign}</div>
                    <div style={{ fontSize: '12px', color: '#9ca3af', marginTop: '4px' }}>{flight.dep_iata || '?'} ➔ {flight.arr_iata || '?'}</div>
                  </div>
                  <div style={{ background: 'rgba(239, 68, 68, 0.2)', color: '#ef4444', padding: '8px 12px', borderRadius: '5px', fontWeight: 'bold', fontSize: '14px' }}>+{flight.delayed} mins</div>
                </div>
              ))
            )}
          </div>
      ...
      {activeTool === 'nearby' && (
        <div style={{ marginTop: '20px' }}>
          <p style={{ color: '#9ca3af', fontSize: '14px', marginBottom: '15px' }}>
            Act as a local Air Traffic Controller. Find planes within a 200km radius of your exact location.
          </p>
          <button
            onClick={onDetectLocation}
            style={{ width: '100%', background: '#3b82f6', color: 'white', fontWeight: 'bold', border: 'none', padding: '12px', borderRadius: '5px', cursor: 'pointer', marginBottom: '20px', boxShadow: '0 4px 10px rgba(59, 130, 246, 0.3)' }}
          >
            {userLocation ? "📍 Location Locked" : "📡 Detect My Location"}
          </button>

          {userLocation && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <div style={{ color: '#10b981', fontWeight: 'bold', marginBottom: '5px' }}>
                ✈️ Overhead Flights: {flights.length}
              </div>
              {flights.map((flight, idx) => (
                <div key={idx} style={{ background: '#1f2937', padding: '15px', borderRadius: '8px', borderLeft: '4px solid #10b981' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                    <span style={{ fontWeight: 'bold', color: '#34d399' }}>{flight.callsign || flight.flight_iata || 'Unknown Flight'}</span>
                    <span style={{ fontSize: '12px', color: '#9ca3af', background: '#111827', padding: '2px 8px', borderRadius: '10px' }}>Alt: {flight.altitude}m</span>
                  </div>
                  <div style={{ fontSize: '13px', color: '#d1d5db' }}>
                    Speed: {flight.velocity} km/h • Heading: {flight.trueTrack}°
                  </div>
                </div>
              ))}
            </div>
